...
**포함** — `courseCategory` 3종, 총 528과목, 성취기준 47,625건(고교 전체의 93.8%), 17개 계열(별책23~39와 1:1). 과목 수는 2026-08-08 빌드에서 상류 실측으로 확정(설계 시 추정치 159/387/2는 오집계였음):

| courseCategory | 한국어 | 과목 수 (실측) |
|---|---|---|
| major-general | 전공일반 | 216 |
| major-practical | 전공실무 | 309 |
| specialized-common | 전문공통 | 3 |

**코드 형식은 카테고리와 1:1이 아니다** (빌드 실측): 2단 `[약칭 NN-NN]`(예: `[간기 01-01]`)과 3단 `[약칭 NN-NN-NN]`(NCS 대응, 예: `[3개 01-01-01]`)이 전공일반 과목 안에도 섞여 있다(v0.1 수록 성취기준의 약 78%가 3단). 코드 구조 판정은 카테고리가 아니라 코드 자체의 형태로 한다.

**제외**: 보통교과·특목 계열(중등판 수록), 중→고 전이·과목 선후 데이터(전문교과에 없음).

**단계 출시**: 아키텍처는 전체 47,625건을 감당하도록 설계하되 출시는 단계적으로 한다.

- v0.1.0 — 전공일반 216과목 + 전문공통 3과목 = 219과목(성취기준 8,425건, 2026-08-08 빌드 실측). 17권 PDF의 해당 장만 추출하면 된다.
...
│   │   ├── curricula.json    # 528과목 색인 (계열 슬러그·카테고리·성취기준 수)
...
| 전문교과 2단 | `[약칭 NN-NN]` | `[간기 01-01]` | 괄호 안 공백, 학년 접두 없음 |
| 전문교과 3단 | `[약칭 NN-NN-NN]` | `[3개 01-01-01]` | NCS 3단계(능력단위-요소-준거 대응) |

주의(빌드 실측): 2단/3단 형식은 전공일반/전공실무 카테고리와 1:1이 아니다 — 전공일반 과목에도 3단 코드가 다수다. 코드 구조 해석(`parseVocationalCode`)의 결과 명칭은 카테고리가 아니라 형식을 나타내는 값(`two-level`/`three-level-ncs`)을 쓴다.
...
## 후기 — 현행과 달라진 지점 (2026-08-11 추가)

이 문서는 v0.1 착수 전 승인된 설계다. 당시 판단 근거를 보존하기 위해 **본문은 수정하지 않는다.** 아래는 이후 구현에서 달라진 지점이며, 서술이 충돌하면 **코드와 테스트가 기준**이다.

### 단계 출시가 v0.2에서 종료됐다

18~40행이 전제한 단계 출시(v0.1 전공일반+전문공통 → v0.2.0+ 전공실무를 계열 단위로)는 v0.2가 17계열을 한 번에 수록하면서 첫 단계 만에 끝났다. 결과는 528과목 · 성취기준 47,625건 · 원문 100% · **미수록 계열 0건**이다.

따라서 다음 서술은 현행과 다르다.

| 행 | 이 문서의 서술 | 현행 |
|---|---|---|
| 85 | 미수록 계열은 `included: false`로 존재하며 조회 시 "v0.x에서 추가 예정" 안내 | 미수록 계열이 존재하지 않음 |
| 124 | `list_major_fields`가 미수록 계열을 투명하게 안내 | 정적 안내 `note` 제거 (v0.2 Task 5) |
| 163 | 미수록 계열 조회 시 수록 예정 안내 | 해당 상태 자체가 없음 |
| 172 | mcp-publisher "미설치 상태 주의" | 2026-08-11 설치 완료 |

### 응답 스키마에서 제거된 필드 (2026-08-11)

전 범위 수록으로 값이 상수가 되어 소비자를 오도하던 필드를 제거했다. 실측 근거: `curricula[].included`는 528건 전부 `true`, `majorFields[].includedCategories`는 18계열 전부 동일한 배열, `dataAvailable`은 항상 `true`.

- `list_major_fields` — `includedCategories`, `dataAvailable`
- `list_curricula` — 계열 요약의 `includedCourseCount`, 과목 레코드의 `included`
- `ensureScope`의 `c.included &&` 조건

**데이터 파일(`data/kr/**`)과 파이프라인은 그대로 두어 이 필드들을 계속 생성한다.** 재생성에는 상류 원본과 별책 PDF 17권이 필요한 반면, 서버 응답 계층 정리만으로 목적(오도하는 상수 필드 제거)이 달성되기 때문이다.

### 남은 잔재 (미처리)

- `aboutText`의 `수록 과목 528/528` 표기 — `counts.curricula`와 `counts.curriculaIndexed`가 같은 값이 됐다.
- `data-store.ensureField`의 "아직 수록되지 않았습니다" 분기 — 모든 계열에 데이터가 있어 도달 불가. 실패하는 테스트를 쓸 수 없어 손대지 않았다.
