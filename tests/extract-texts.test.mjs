...
// ── v0.2 최종 리뷰 C2: 괄호구가 줄바꿈으로 갈라져 닫는 괄호가 줄 첫머리에 온 경우 ──

test('컷 억제: 줄바꿈으로 갈라진 닫는 괄호를 "글자)" 헤딩으로 오인하지 않는다', () => {
  // 실측 [컴생 07-02-04] — PDF가 '밀링(머시닝센 / 터)'로 조판해 '터) '가 줄 첫머리에 온다.
  const pdf = [
    '[컴생 07-02-04] CAM 시스템을 사용하여 CNC 밀링(머시닝센',
    '',
    '     터) 가공 데이터를 생성할 수 있다.',
    '[컴생 07-02-05] 다음 성취기준.',
  ].join('\n');
  const { texts, failures } = extractTexts(pdf, ['[컴생 07-02-04]', '[컴생 07-02-05]']);
  assert.equal(failures.length, 0);
  assert.equal(
    texts.get('[컴생 07-02-04]'),
    'CAM 시스템을 사용하여 CNC 밀링(머시닝센 터) 가공 데이터를 생성할 수 있다.'
  );
});

test('컷 억제: 영문 약어 괄호가 갈라진 경우도 본문이 온전하다', () => {
  // 실측 [선통 07-02-01] — 'LRIT(… Ships 선박 장거리 식별 추적 장 / 치) 장비의 …'.
  const pdf = [
    '[선통 07-02-01] LRIT(Long-Range Identification and Tracking of Ships 선박 장거리 식별 추적 장',
    '     치) 장비의 정상 작동 여부를 확인할 수 있다.',
    '<성취기준 적용 시 고려 사항>',
  ].join('\n');
  const { texts } = extractTexts(pdf, ['[선통 07-02-01]']);
  assert.equal(
    texts.get('[선통 07-02-01]'),
    'LRIT(Long-Range Identification and Tracking of Ships 선박 장거리 식별 추적 장 치) 장비의 정상 작동 여부를 확인할 수 있다.'
  );
});

test('컷 억제 이후에도 뒤따르는 진짜 헤딩은 그대로 잘린다', () => {
  // 괄호가 닫힌 뒤의 '나) 곤충의 생리 및 생태'는 앞에 열린 괄호가 없으므로 진짜 헤딩이다.
  const pdf = [
    '[패소 05-01-02] 마스터 패턴, 프레스(몰',
    '     드) 재단을 위한 철형을 제작할 수 있다.',
    '나) 곤충의 생리 및 생태',
    '이 헤딩 이하는 잘려야 한다.',
  ].join('\n');
  const { texts } = extractTexts(pdf, ['[패소 05-01-02]']);
  assert.equal(
    texts.get('[패소 05-01-02]'),
    '마스터 패턴, 프레스(몰 드) 재단을 위한 철형을 제작할 수 있다.'
  );
});

test('컷: 코드 행 컷은 하이픈이 대시 이형으로 조판된 이웃 코드 행도 잡는다', () => {
  // codePattern은 이미 대시 이형을 허용한다 — 코드 행 컷도 같은 정책이어야 한다.
  const pdf = '[전기 01-01] 회로를 구성한다.\n[네일 04–01-02] 이 행은 대시 이형이지만 잘려야 한다.';
  const { texts, failures } = extractTexts(pdf, ['[전기 01-01]']);
  assert.equal(failures.length, 0);
  assert.equal(texts.get('[전기 01-01]'), '회로를 구성한다.');
});
